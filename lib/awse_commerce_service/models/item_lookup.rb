...
module AwseCommerceService
  ...
  class ItemLookup < BaseModel
    ...
    # TODO: Write general description for this method
    # @return [List of ItemLookupRequest]
    ...
    def self.from_hash(hash)
      ...
      shared = ItemLookupRequest.from_hash(hash['Shared']) if hash['Shared']
      # Parameter is an array, so we need to iterate through it
      request = nil
      unless hash['Request'].nil?
        request = []
        hash['Request'].each do |structure|
          request << (ItemLookupRequest.from_hash(structure) if structure)
        end
      end

      # Create object from extracted values.
      ItemLookup.new(marketplace_domain,
                     aws_access_key_id,
                     associate_tag,
                     validate,
                     xml_escaping,
