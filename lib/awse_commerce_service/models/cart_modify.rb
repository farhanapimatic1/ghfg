...
module AwseCommerceService
  ...
  class CartModify < BaseModel
    ...
    # TODO: Write general description for this method
    # @return [List of CartModifyRequest]
    ...
    def self.from_hash(hash)
      ...
      shared = CartModifyRequest.from_hash(hash['Shared']) if hash['Shared']
      # Parameter is an array, so we need to iterate through it
      request = nil
      unless hash['Request'].nil?
        request = []
        hash['Request'].each do |structure|
          request << (CartModifyRequest.from_hash(structure) if structure)
        end
      end

      # Create object from extracted values.
      CartModify.new(marketplace_domain,
                     aws_access_key_id,
                     associate_tag,
                     validate,
                     xml_escaping,
