...
module AwseCommerceService
  ...
  class CartCreate < BaseModel
    ...
    # TODO: Write general description for this method
    # @return [List of CartCreateRequest]
    ...
    def self.from_hash(hash)
      ...
      shared = CartCreateRequest.from_hash(hash['Shared']) if hash['Shared']
      # Parameter is an array, so we need to iterate through it
      request = nil
      unless hash['Request'].nil?
        request = []
        hash['Request'].each do |structure|
          request << (CartCreateRequest.from_hash(structure) if structure)
        end
      end

      # Create object from extracted values.
      CartCreate.new(marketplace_domain,
                     aws_access_key_id,
                     associate_tag,
                     validate,
                     xml_escaping,
