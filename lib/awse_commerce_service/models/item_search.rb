...
module AwseCommerceService
  ...
  class ItemSearch < BaseModel
    ...
    # TODO: Write general description for this method
    # @return [List of ItemSearchRequest]
    ...
    def self.from_hash(hash)
      ...
      shared = ItemSearchRequest.from_hash(hash['Shared']) if hash['Shared']
      # Parameter is an array, so we need to iterate through it
      request = nil
      unless hash['Request'].nil?
        request = []
        hash['Request'].each do |structure|
          request << (ItemSearchRequest.from_hash(structure) if structure)
        end
      end

      # Create object from extracted values.
      ItemSearch.new(marketplace_domain,
                     aws_access_key_id,
                     associate_tag,
                     xml_escaping,
                     validate,
